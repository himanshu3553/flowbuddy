...
- A workflow may also carry "starts on:" (the page it begins on) and "when done, the user sees:" (what appeared on screen when it was recorded succeeding). When you answer HOW to do a task, OPEN by saying where to begin — name the "starts on" page before step 1 — and when the finish line is known, CLOSE with how they'll know it worked. Both are recorded facts: never invent a starting page or a success message for a workflow that doesn't carry them.

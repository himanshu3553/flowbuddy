...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    if (msg?.cmd === 'start') {
      sendResponse(await onStart(msg.backendUrl));
...
async function onStart(backendUrl: string): Promise<{ ok: boolean; error?: string }> {
  ...
  await setRec({ recording: true, tabId: tab.id, windowId: tab.windowId, startTime, backendUrl });
...
async function finalize(): Promise<void> {
  ...
  try {
    ...
    const backendUrl = (rec.backendUrl || 'http://localhost:8787').replace(/\/$/, '');
    const res = await fetch(`${backendUrl}/sessions`, { method: 'POST', body: fd });
    const json = await res.json();
    await kvClear();
    if (json?.renderUrl) {
      await chrome.tabs.create({ url: json.renderUrl });
    } else {
      console.error('Upload finished but no renderUrl returned:', json);
    }
  } catch (e) {

...
| [`phase-2-reason.md`](docs/phase-2-reason.md) | **P2-M5 (Reason / diagnostic reasoning) — 📝 design LOCKED 2026-07-09, ready to build.** Sense locates, Reason diagnoses: ask-time structured page-state capture (web-standards only; values masked; end-user-silent, founder toggle + disclosure) + the founder's expected-state step screenshot → agentic read-tool reasoning ("why is this button disabled?"). Supersedes the old fuzzy-fallback idea; the loop is the skeleton Phase 4 inherits. |
| [`phase-4-autopilot.md`](docs/phase-4-autopilot.md) | **Phase 4 (Autopilot / agentic execution)** — the copilot executes approved workflows in the end-user's live session (grounded actions: only recorded + approved workflows, human-in-the-loop, safe-stop). Draft — candidate modules P4-M0…M3 + design questions; **gated behind Phase 3** (consumes its replay engine + validated-current certification). |
...
> **Version 1 = a pure copilot arc (restructured 2026-07-08):** Phase 1 **Copilot** (✅ shipped) → Phase 2 **Sense** (✅ built + user-verified 2026-07-09 — in-context help: read-only locator probe → workflow/step localization → positional answers; **+ P2-M5 Reason** — diagnostic reasoning, design locked 2026-07-09, ready to build) → Phase 3 **Self-validation** (sandbox replay, drift — the moat) → Phase 4 **Autopilot** (agentic execution over Phase 3's replay core). **Version 2** holds the by-products + depth: the **Help Portal & Articles track** (renders approved workflows as articles — [`docs/v2-portal.md`](docs/v2-portal.md)), narration/video capture modalities, and the deferred depth buckets. Map: [`docs/roadmap.md`](docs/roadmap.md).

...
- **Mode 1 and the agent render knowledge items SEPARATELY, on purpose.** `engine.ts formatItems` is the agent's; mode 1 and the diagnostic path inline their own. The freeze is about TUNING, not capability: something wrong for every tier (the workflow plan was) lands in all of them deliberately, each measured on its own baseline. Adding to one and forgetting the other leaves the safety floor answering worse than the tier above it for no good reason.

...
| [`phase-2-sense.md`](docs/phase-2-sense.md) | **Phase 2 (Sense / in-context help)** — the copilot localizes the end-user to **workflow + step** (ask-time read-only locator probe; hybrid: client scores → top-k hypotheses ride `/answer` → the LLM disambiguates with the question) and answers **positionally** (unstick step k → remaining path, step-level citations, tie → "X or Y?"). Modules P2-M0…M4; design locked 2026-07-08; next up after Phase 1. |

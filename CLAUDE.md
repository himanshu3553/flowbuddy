...
- **`KnowledgeItem.text` is never written without its re-embed — text and vector move TOGETHER, or the save fails whole.** The worker matches vectors to rows BY TEXT when persisting them, so a text write that skips embedding desyncs retrieval invisibly: bad answers, no error anywhere. The only writers are the worker and Studio's edit actions; any new edit surface must follow the same rule, may touch only the FOUNDER'S LAYER — prose, the step's frame ONLY when validated against the recording's own manifest (`bbox` follows the picture; no re-embed since `text` is untouched), and which captured moments are INCLUDED as steps (delete/restore ride `KnowledgeSource.stepInclusions` by anchor; a restored step's anchor resolves from the REAL event, so a step with no captured event is impossible) — never the event citation (`keyEventId`, `sourceEventIds`, `route`, `evidence`), and must stamp the edit — the stamp plus `data.editedFields` is what carries a founder's edits through a reprocess (re-attached by `data.keyEventId`; stamped title/description fields are human-owned and the worker keeps them).

...
flowchart TB
    subgraph BUILDER["🛠️ BUILDER SIDE — record &amp; curate (async · BullMQ)"]
        ...
        ING["<b>INGESTION API</b><br/>api · server.ts<br/>/v1/uploads/sign → presigned PUT URLs (api never touches artifact bytes)<br/>/v1/sessions → manifest + audio + leftovers, idempotent per uploadId<br/>upsert KnowledgeSource · enqueue job"]
...
- **Transport (2026-07-27) — the bundle is no longer one all-or-nothing POST.** While recording, the recorder batches artifact paths to **`POST /v1/uploads/sign`** (recorder token; ≤100 paths per call) and PUTs each screenshot / DOM snapshot **directly to object storage** with a 900 s presigned URL — the api never touches those bytes. **Stop** sends only the manifest, the audio, and any artifact storage never confirmed.
...
- **Degradation.** If signing fails (offline, auth, an older server), nothing is lost — unconfirmed artifacts simply ride the Stop bundle exactly as before.

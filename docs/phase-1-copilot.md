...
- **Widget & SDK (P1-M7):** `sync-copilot.js` (esbuild → ~7 kB IIFE); shadow-DOM launcher + chat panel; **neutral default theme matching Studio**, host-themeable via `data-sync-accent` + `data-sync-position` (plus `data-sync-title`/`-greeting`); renders answers + citations + decline/error states; `demo/index.html` for local testing.
...
*Redesigned twice: 2026-06-26 (`f5197c0`) onto **Tailwind + shadcn/ui** (neutral); then **2026-06-28** rebuilt under the **indigo brand** from the Claude Design handoff ([`design_handoff_sync_studio/`](design_handoff_sync_studio/README.md), branch `ui-change-copilot`) — Plus Jakarta Sans + JetBrains Mono, indigo tokens derived via opacity, a 6-item nav, and **empty/loading/error states on every screen**. Web-only change.*
...
- **Shell & IA:** persistent sidebar (Sync mark, workspace switcher, user footer w/ sign-out) over a 6-item nav — **Home · Recordings · Knowledge Base · Copilot · Analytics · Settings** — + a **per-page header** (title + subtitle + actions); responsive (mobile drawer + mobile top bar).
  ...
  - **Copilot** (`/dashboard/copilot`): **tabs** (Install / Settings / Appearance) — embed snippet + copy, "not detected yet" listening state + checklist, public key + rotate, origin allowlist, grounding & trust controls (Cite / decline-threshold are UI-only previews) — plus a static **end-user widget preview** and real Copilot activity. *(Embed-detection is not wired; the F17 origin-blocked error state is deferred pending a blocked-origin signal.)*
  - **Analytics** (`/dashboard/analytics`): 7-day answer-quality metrics, answered-vs-declined chart, coverage-gaps "record this next" table, recent declines, "resolved without a human" + empty state. *(Tickets-deflected is shown as ≈answered; top-workflows-by-citations is a placeholder — no citation logging yet.)*

...
**Build sequence (V1, locked 2026-06-21):** **M6** (KB layer ✅) → **M6.1** (curated generation ✅) → **M7** (prompt-to-article ✅) → **M8** (Render deploy — *next*). **Narration-only capture is removed from V1 → see "Version 2 backlog" at the end of this section.**
...
- **✅ DONE 2026-06-21 — as built & verified (built on the three locked leans):** **(1) retrieval** = keyword shortlist (Postgres `contains`/`mode:insensitive` over `KnowledgeItem.text`, stopword-filtered, ≤40, workspace-wide) → LLM selects + synthesizes-or-declines; **(2) linkage** = prompt-grounded `Article.sessionId = null` (spans recordings), screenshots/highlights resolved **per-event source** via an `eventId→{sourceId,file,viewport}` index; **(3) decline** = LLM judges coverage (or 0-hit short-circuit) → `CoverageGap`. New: `synthesis/src/prompt.ts` (`promptToArticle`, multi-source `PromptArtifactReader`), `CoverageGap` model + migration `20260621103301_coverage_gap`, web `lib/prompt-actions.ts` (`generateFromPrompt` + `resolveCoverageGap`), client `app/dashboard/prompt-box.tsx`, dashboard "Text → Article" card + coverage-gaps list, `.row` CSS utility (was referenced but undefined). **Verified:** "create a new project" → COVERED, 7-step draft; "reset password / 2FA" → DECLINED + reason. 7/7 typecheck + web `next build` clean. **Committed `e5e6120` (pushed).**

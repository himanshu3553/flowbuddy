...
| [`agent.md`](build/agent.md) | The unified agent — the locked decisions, the two modes, the acting layer and the goal layer. |
| [`execution-contracts.md`](build/execution-contracts.md) | One evidence layer per recording — entry, steps, outcome. Answers state it, Sense diagnoses with it, the acting run verifies against it. |

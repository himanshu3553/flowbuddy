...
- **Status:** [`roadmap.md`](../roadmap.md) §12 — this doc records the direction, the feasibility assessment and the transport analysis (§3–§6), never what is built or when it is scheduled. What it *does* fix: transport is **recommended, not locked** — one two-layer export schema (§5), rendered as **remote MCP (v1 lead) + markdown (v1 rider) + WebMCP via the widget (the prepared bet)**; a bespoke REST API is skipped as a product. The dates behind each of those live in the decision log (§9).
- **Phase name is provisional** ("Interop"); rename freely when the phase is designed.
- **Companion docs:** the KB this phase exports → [`architecture.md`](../product/architecture.md) + [`kb-step-distillation.md`](kb-step-distillation.md) · the compiler head start (P2-M0 sense plan) → [`sense-and-reason.md`](sense-and-reason.md) · first-party execution (our own hands) → [`agent.md`](agent.md) · the goal brain → [`agent.md`](agent.md) · freshness/certification → [`roadmap.md`](../roadmap.md) §4 · the improvising-agent contrast → [`competitive-claude-chrome.md`](../product/competitive-claude-chrome.md) · the buyer-side sibling (FlowBuddy's *own* agent) → [`company-agent.md`](company-agent.md)
...
4. **One KB, another audience.** Extends the locked mental model unchanged: `ONE raw KB → per-target approval/visibility → { Copilot, Portal (V2), Agents (this phase) }`. No new pipeline, no second source of truth — one more audience flag on the same approval model (P4's acting flag already rides that row; V2's `portal` flag is designed the same way).
...
**⚠️ The head start moved once P4 shipped — it is no longer the probe.** The acting layer compiles a *fuller* artifact than the sense plan: the **`ExecutionPlan`** ([`agent.md`](agent.md) §A2.2), compiled per actable workflow, carrying step verb, instruction, route pattern, ranked locators, post-route, **outcome evidence**, input slots (label + sensitive flag) and destructive flags — content-hashed, versioned, and re-verified against workflow identity on reprocess. That is most of the §5 instructional layer *and* all of its machine layer, already running. **P6-M0 should generalize the `ExecutionPlan`, not `sense-plan.ts`** — exporting is closer to widening something that already runs than to building anything. What genuinely remains is audience shaping: workflow goal/preconditions, the freshness metadata, and dropping the first-party assumptions (consent pinning, run TTL) an external consumer has no use for. Generalizing the probe instead redoes work that exists.

| Piece | Status |
|---|---|
| Approved-only workflow compiler | ✅ exists — **P4's `ExecutionPlan` is the one to generalize**; `sense-plan.ts` is the narrower, probe-shaped sibling |
| Trust-gate pattern (audience flags on the approval model) | ✅ exists — P4's per-workflow **acting flag rides the approval row** today; `portal` (V2) is designed the same way; `agents` is one more value + a workspace opt-in toggle |
...
| **V3 Company agent** ([`company-agent.md`](company-agent.md)) | FlowBuddy's *own* grounded browser-agent extension, buyer-side — runs the tools a company uses from its own recorded KB | **The first consumer of this phase's export seam** — P6 feeds *their* agents; V3 brings *ours* |

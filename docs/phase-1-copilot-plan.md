...
### P1-M10 — Feedback loop & analytics — ✅ DONE (2026-06-23)
*Built: `CopilotQuery` model + `CoverageGap.source` (migration `…_copilot_query`); `/answer` logs each Q&A and returns a `queryId`; new `POST /v1/copilot/feedback` (tenant-scoped thumbs); copilot gaps tagged `source=copilot`; widget renders 👍/👎 on answers and posts feedback; Studio `/dashboard/copilot` "Copilot activity" (total · answered% · 👍/👎 · recent) + dashboard gaps show source. Verified: logging, tenant-scoped feedback, aggregates, gap source default vs copilot.*

...
- **Recordings now upload artifacts straight to object storage while you record** (the api signs a PUT URL per artifact and never touches the bytes). Two local implications: **(1)** `R2_ENDPOINT` must be an address the **browser** can reach — the default `http://localhost:9000` (MinIO) is correct; a docker-internal hostname would sign URLs nothing outside the compose network can use. **(2)** **MinIO is more permissive than Cloudflare R2** — it ignores the request checksums R2 enforces, so a broken signed URL passes locally and fails only in the cloud. A green local recording is **not** proof the upload path works on R2; verify on the dev deploy ([`e2e-testing.md`](e2e-testing.md) Level 2).

...
| **The unified agent** | **The decision to stop shipping those as separate features.** Not "a copilot, plus a walkthrough feature, plus an autopilot button" — one assistant that reaches for whichever it needs, moment to moment. |
...
One conversation. No menus, no modes, and the user never decides in advance what *kind* of help they want. **That fluidity is the whole point** — and it is unreachable while Tell, Show, and Do are separate mechanisms with separate entry points.
...
**What it changed is the packaging** — and one real idea got thrown away: that the **user** picks a mode. Now the agent picks, per message, from a set of grounded tools the founder has approved.

### Where we actually are

| | |
|:---|:---|
| **Done, live** | Answering · knowing where the user is (Sense) · diagnosing why they're stuck (Reason) · guided walkthroughs (P4-M0) |
| **Next** | **P5-M0** — make the conversation survive page changes. Today it resets on navigation, including navigations the walkthrough itself causes. |
| **After that** | Turn the answering path into the agent loop, **read-only tools first** — no new risk surface, and it proves or falsifies the whole thesis cheaply |
| **The big one** | Teaching it to actually click and type (**P4-M2**) — the last piece and the riskiest |
...
2. **Tier-3 certification bar** — require Phase-3 green validation before any hands-off run, or accept interim signals (recent successful walkthroughs/runs) until P3 lands? *(= [`phase-5-converse.md`](phase-5-converse.md) §5 Q10.)*
3. **Chaining scope for v1** — single-workflow goals first, chains later? *(= §5 Q5.)*
4. **Founder posture control shape** — three presets over the existing five toggles, or a different cut? Does the cost ceiling ship with it?
5. **Cross-origin iframe UX** — is "highlight the region + Continue" enough, or does the payment case want a bespoke affordance?

## 8. Migration path

Each step ships standalone value; step 2 is where the thesis is proven and the transport question gets settled.

1. **P5-M0 — conversational foundation.** Chat persistence + continuity retrieval. Needed under every version of this, zero new risk surface, and it fixes a live bug: the chat dies on navigation today, including navigations the walkthrough itself causes.
2. **Promote `diagnoseFromKB` to the main loop, read-only tools first** — `search_kb`, `get_workflow`, `where_am_i`, `read_page_state`, `product_profile`, `ask_user`. Nothing acts, so **the risk surface does not grow**; Tell + diagnosis + clarifying questions unify immediately.
3. **Add the shipped zero-acting client tools** — `highlight_step`, `run_walkthrough`. Tell and Show become one mechanism, and mid-walkthrough questions start working.
4. **`execute_step` plugs in as one more tool** when P4-M2 exists, gated by P4-M1's eligibility signals (and Phase 3's certification when it lands).

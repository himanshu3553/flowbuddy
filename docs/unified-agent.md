...
| **Done 2026-07-29** | **The copilot answers the question it was just asked** — with any earlier turn in the thread it was answering the PREVIOUS one (0/10 → 10/10 both modes). Plus the tool loop's de-duplication re-keyed on arguments, off-screen workflow lookup, and **every answer now records which engine produced it, in how many rounds, with which tools.** | 1 + 2 |
...
|:---:|:---|:---:|:---|
| **1** | **P5-M0 — conversational foundation.** Chat persistence + continuity retrieval. Needed under every version of this, and it fixes a live bug: the chat dies on navigation today, including navigations the walkthrough itself causes. **Ships to mode 1 too — hygiene, not an agent feature.** *(Two amendments: typed message kinds in the persisted format from day one, so D3's chat-supplied values are excludable later without a storage migration; and build its persistence deliberately as the transport prototype for §7 Q1.)* <br>**✅ CUT 1 BUILT 2026-07-26** (user-verified E2E, in production) — **both amendments landed.** `widget/src/session.ts` is a slot-based store (`walkthrough` · `chat` · later `agent-run`) owning versioning/key-scoping/stamps/TTL/discard, extracted at the second consumer rather than guessed at the first; the walkthrough refactored onto it (`flowbuddy.walkthrough.v2`). `MsgKind` + a `PERSISTED_KINDS` allowlist are the persistence boundary, so `user.value` is excluded by never being added. **Q1 now has a working reference implementation of option (c)** — see below. Cut 2 (continuity bias) shipped the same day; cut 3 (condensation) was **dropped** — cut 2 took its common case for free and the agent absorbs the rest. | **1 + 2** | none |
...
1. **The prompt rewrite** — step 2 touches the fast path *every* question rides. Answers that are good today can degrade in ways nobody notices for weeks. Automated coverage is partial — `pnpm test` covers the shared loop and the pure seams, and `scripts/copilot-baseline.mjs` measures answer decisions against a fixed question set (multi-turn cases included), but the diagnostic path has none; re-run the Sense/Reason/walkthrough legs of [`e2e-testing.md`](e2e-testing.md) before calling it done. This is the moment that backlog item earns its keep.

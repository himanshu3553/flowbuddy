...
2. ✅ **[synthesis/src/index.ts](../packages/synthesis/src/index.ts) `StepItemData` / `decodeStepData`** *(fixed 2026-07-05 → **moot since 2026-07-07**)* — the banner was added, then the whole parked article engine (incl. `decodeStepData`/`StepItemData`) was **removed with the Phase-2 sweep** (`844a316`; [`phase-2-portal.md`](phase-2-portal.md) §7). Nothing decodes the legacy `{ event, narration }` shape anymore — the concern no longer exists in the tree.
...
7. Then proceed to whatever's next on a clean foundation. *(Since taken up: **pgvector ✅ shipped 2026-07-07**, **Phase-2 direction set to workflows-as-articles** + the engine swept, **Approach-B real-widget tester ✅ merged**, **structured logging ✅ shipped** — so "next" is now Phase-2 portal + the remaining P1 debts §3.7/§3.9.)*

Items deliberately **not** recommended at review time: ~~pgvector (wait for the trigger)~~ *(since **shipped** 2026-07-07 — the trigger call was overtaken by the decision to build hybrid retrieval)*, PII Cut 2 (already correctly deferred — now folded into Phase-2 P2-M5), widget a11y & multi-seat auth (pre-beta list), R5/R12/R13 *(R13 since **shipped** 2026-07-06)*.

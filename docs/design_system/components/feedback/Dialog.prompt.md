A centered modal on a dim backdrop — the help dialogs ("How Sync works", "How to record") and onboarding. Renders absolutely inside a positioned app frame.

```jsx
<Dialog open={open} onClose={close} title="How Sync works"
  ...
  <StepItem index={2} icon="menu_book" title="Sync builds your knowledge base" desc="…" last />
</Dialog>
```

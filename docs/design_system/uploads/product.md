...
- **Moats (the actual product):**
  ...
  2. **Self-validation / freshness** — knowledge that re-checks itself and flags drift. Hardest to copy; directly answers "products change faster than docs." (Also the namesake of the original working name "Sync": keeping the KB *in sync* with the product.)
...
FlowBuddy ships as **four distinct surfaces** over one shared structured knowledge base. The **copilot is the headline**; the portal is a decoupled by-product.

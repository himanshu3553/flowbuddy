...
**Chat persistence across navigations (P5-M0 cut 1 — built 2026-07-26, needs first verification):**
...
4. **Follow-ups now carry across pages.** After navigating, ask *"and then what?"* — the server receives the pre-navigation turns in `history` (this is the one behavioral change: `history` now spans navigations). Watch for answers that get *worse* from too much stale context.

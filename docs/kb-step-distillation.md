...
> Note: at design time a step-distillation engine already existed in-tree — `synthesize.ts` (the parked Phase-2 article generator) turned events into clean prose steps; it was never wired into the copilot path, and the copilot-focused distiller built here reused its patterns. *(That parked engine was **removed 2026-07-07** — superseded by workflows-as-articles, [`phase-2-portal.md`](phase-2-portal.md) §7.)*
...
- **Built:** `buildWorkflowKB` in [`index.ts`](../packages/synthesis/src/index.ts) (transcribe → align → **clean** → segment → **distill** → distilled steps grouped by workflow). [`worker.ts`](../packages/api/src/worker.ts) now calls it and persists **distilled steps** (clean `text` = `distilledStepText` + `data: { instruction, detail, route, narration, screenshotFile, bbox }` + `segmentIndex`/`segmentTitle`, `orderIndex` within workflow). The raw 1:1 persistence + the read-back/tag round-trip are gone. `buildKB`/`segmentItems` stayed exported for the parked Phase-2 article engine until both were **removed 2026-07-07** with the Phase-2 sweep ([`phase-2-portal.md`](phase-2-portal.md) §7).
...
> **Parked Phase-2 impact — OBSOLETE (2026-07-07):** the parked article engine (`generate-actions.ts`, `prompt-actions.ts`) read raw events from `KnowledgeItem.data.event`, which new recordings no longer store — it was never re-sourced because the whole engine was **removed 2026-07-07**, superseded by workflows-as-articles ([`phase-2-portal.md`](phase-2-portal.md) §7).

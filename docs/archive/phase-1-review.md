...
2. ✅ **[synthesis/src/index.ts](../../packages/synthesis/src/index.ts) `StepItemData` / `decodeStepData`** *(fixed 2026-07-05 → **moot since 2026-07-07**)* — the banner was added, then the whole parked article engine (incl. `decodeStepData`/`StepItemData`) was **removed with the Phase-2 sweep** (`844a316`; [`v2-portal.md`](../v2-portal.md)). Nothing decodes the legacy `{ event, narration }` shape anymore — the concern no longer exists in the tree.

...
> **📁 ARCHIVED (historical record).** This 2026-07-03 review served its purpose — nearly every finding landed (annotated ✅ inline below). It is kept as the audit trail of what was reviewed and fixed. The **still-open items** it surfaced now live as the **Phase 1 backlog** in [`roadmap.md`](../roadmap.md) §9 — that is the live list; this file is the detail behind it.
...
**Remediation landed 2026-07-06 (`1bba47b` on `dev`, user-verified E2E):** ✅ **all of P0 §2.1–2.6**, ✅ **§2.7** (origin normalization + Studio warning), ✅ **§3.1/3.2** (retrieval consolidated into `@flowbuddy/synthesis` `retrieval.ts`; `api/src/copilot.ts` deleted, `listApprovedItems` retired), ✅ **§3.3** (transcription degrades, not fails), ✅ **§3.4** (graceful shutdown), ✅ **§4.5** (KB-page honesty reword) — plus a new `KnowledgeSource.updatedAt` column (migration) powering the §2.2 "Stalled" surface. ⏸️ **§3.5 deferred** (built then reverted — see §3.5).

**Landed since (as of 2026-07-08):** ✅ **pgvector shipped** (P1-M3, `c357e2e`, 2026-07-07) as hybrid keyword∪pgvector RRF — this **overtakes the review's "wait for the trigger" stance** in §3.2/§7 (the retrieval consolidation §3.1 gave it exactly one landing spot, as predicted). ✅ **§3.8 observability — the structured-logging half is done** (`5d3fa13`, 2026-07-08): the new `@flowbuddy/logger` (Pino) now carries every Node service with `service`/context fields + secret redaction (Sentry/error-aggregation still open). ✅ **§4 nit #4** (answer-call `temperature`) landed inside the §2.4 cheap fix (`temperature 0.2`). ✅ **Approach B merged** (`8d34749`, 2026-07-08) → `copilot-preview-actions.ts` gone, one answer path (the §3.1 "two enforcement implementations" risk is fully closed). 🗑️ **Phase-2 sweep** (`844a316`, 2026-07-07) removed the parked article engine + `Article`/`Step` tables — so **§6.2's `StepItemData`/`decodeStepData` banner is moot** (that code no longer exists). **Still open:** ~~§2.8 (production deploy — gated on the first external embed)~~ *(✅ executed 2026-07-23 — [`deploy.md`](../ops/deploy.md))*, §3.6 signup gate only, §3.7, §3.9 (test layer), §3.8 error-aggregation, remaining §4 nits, §6.5.
...
- 2.8 Free-tier deploy is fine for testing, wrong for a real embed — ✅ executed 2026-07-23 (production launch: the paid two-blueprint stack on flowbuddyai.com — [`deploy.md`](../ops/deploy.md))

---

## 3. P1 — architectural debts to pay before building more on top

### 3.1 Retrieval is implemented twice — collapse to one module — ✅ done 2026-07-06 (`1bba47b`: `@flowbuddy/synthesis` retrieval.ts; api/src/copilot.ts deleted; listApprovedItems retired; preview gains the route-boost)
`api/src/copilot.ts` and `web/lib/copilot-preview-actions.ts` (both since deleted — §3.1 header + the 2026-07-08 header note) each carry their own copy of the STOP list, the keyword shortlist, and `sanitizeHistory`, held together by "this MUST mirror" comments (and they've already drifted: the preview lacks the route-boost). The *no-leak guarantee* — the most important invariant in the product — currently has **two** enforcement implementations (`retrieveApprovedKBItems` in the API, `listApprovedItems` in web).
**Recommendation:** extract a single `retrieval.ts` into `@flowbuddy/synthesis` (it already owns the KB item types): `shortlistItems(items, question, opts)` + `sanitizeHistory` + one approved-items query helper that takes a Prisma client. API and Studio preview both call it. This also gives pgvector (P1-M3) exactly one place to land.
...
**Recommendation:** on connect, delete previous tokens with the same label (`'FlowBuddy Recorder extension'`) so reconnecting rotates rather than accumulates; longer-term, a Settings section listing tokens (label, created, last-used) with revoke.
...
### 3.6 Studio auth hardening — ✅ Cuts 2+3 done 2026-07-06; signup gate deliberately NOT built
...
**Status (2026-07-06):** built after the extension's Web Store publish raised the urgency — **sign-in rate limiting** (5 failed/15 min per email + 20 per IP, in-memory MVP like the copilot limiter; success clears) and **password reset + email verification via Resend** (`lib/email.ts` REST-fetch, no SDK; single-use SHA-256-hashed tokens in the previously-unused Auth.js `VerificationToken` table — no migration; reset = 1 h TTL, verify = 24 h; enumeration-safe + email-request rate-limited; new pages `/forgot-password`, `/reset-password`, `/verify-email`). Verification is enforced **only when `RESEND_API_KEY` is set** — keyless local dev auto-verifies at signup, unchanged. ⚠️ First enable on a deployed DB needs the backfill in render.yaml's comment (pre-existing accounts have `emailVerified = null`). **Deliberately kept OPEN: signup itself** (no invite gate — the user chose to leave signup public and harden around it); revisit the invite/allowlist gate at private beta.
...
### 3.8 Observability before more features — 🔄 pino logging done 2026-07-08 (`5d3fa13`); error-aggregation still open
...
**Status (2026-07-08):** ✅ the **pino logger is built** — `@flowbuddy/logger` (`createLogger('<service>')`, env-driven level, secret redaction, JSON in prod / pretty in dev) now carries `api` (Fastify via `loggerInstance`), `synthesis`, and `web` server-side; Fastify default logging is replaced. Canonical doc: [`dev-setup.md`](../ops/dev-setup.md) §7. ⏳ **Still open:** Sentry-style error aggregation + the per-call latency/token logging in `answerFromKB` (pairs with §2.4's budget counter).
...
| 4 | [copilot.ts (synthesis)](../../packages/synthesis/src/copilot.ts) | ✅ **fixed 2026-07-06** (landed inside the §2.4 cheap fix — `temperature 0.2` + `max_completion_tokens`) — was: no `temperature` on the answer call (defaulted to 1.0) | Set ~0.2–0.3 for consistent answers |
...
2. ✅ **[synthesis/src/index.ts](../../packages/synthesis/src/index.ts) `StepItemData` / `decodeStepData`** *(fixed 2026-07-05 → **moot since 2026-07-07**)* — the banner was added, then the whole parked article engine (incl. `decodeStepData`/`StepItemData`) was **removed with the Phase-2 sweep** (`844a316`; [`portal.md`](../build/portal.md)). Nothing decodes the legacy `{ event, narration }` shape anymore — the concern no longer exists in the tree.
3. ✅ **Doc maps don't list the deploy doc** *(resolved 2026-07-05)*. `deploy-render.md` existed but wasn't listed in [roadmap.md](../roadmap.md) §10 (the doc map) or the CLAUDE.md doc table. *(2026-07-04: `render-reset-and-test.md` was merged into `e2e-testing.md` as its Level-2 section; 2026-07-05: `deploy-render.md` + this review doc were added to both doc maps and the README table.)*

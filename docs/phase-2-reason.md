# FlowBuddy — Phase 2 · P2-M5: Reason (diagnostic reasoning)
...
**Why FlowBuddy wins this game:** every generic bot can at best see the user's page. Only FlowBuddy also has **the founder's recording of the same step succeeding** — screenshot + captured state. Expected-vs-actual comparison is the moat move: *"When this step works, the button is enabled after a valid form; on your screen the email fails format validation and 3 of 4 password rules are unmet."*
...
1. **Stale embedded bundle** — the first live test produced generic hedging because the test app serves a **copied** widget file from its own `public/` dir; it was the pre-Reason build, so no capture ever ran (`CopilotQuery.reasonTrigger` all-NULL was the tell). After any widget rebuild, re-copy **both** `flowbuddy-copilot.js` **and** `flowbuddy-copilot-render.js` into the embedding app.
...
**Diagnosis-quality hardening (2026-07-16 — from the duplicate-email E2E miss):** a live test caught the loop diagnosing "password requirements unmet / button not clickable" while the page actually showed a healthy form, an **enabled** button, and a server-rejection banner ("An account with this email already exists") — the model had the image and still anchored on the founder's recorded step context. Six fixes:
1. **On-page errors first** (prompt) — an error/alert/rejection message in the evidence IS the primary diagnosis: explain it and the way forward before any theory; a complete form + rejection feedback means the *action* was rejected — diagnose the rejection, not the form.
2. **Never claim a state the evidence contradicts** (prompt) — disabled/blocked/not-clickable may only be asserted when PAGE STATE says DISABLED; buttons now report **`enabled` explicitly** in the evidence lines (`elementLine`) so the rule has positive facts to stand on.
3. **Image-first reading order** (prompt) — with the image present, read banners/toasts and checklists' met/unmet marks from the pixels first; PAGE STATE stays ground truth for machine facts; never assert a requirement UNMET without evidence.
4. **`[alert]`-tagged capture pass** (widget `captureSnapshot`) — alert/error surfaces (`role=alert`, assertive live regions, `class*=error|danger|alert`) are collected **document-wide, first**, tagged `[alert]`, with a reserved slice of the text budget — a rejection banner can no longer lose the 40-text budget to marketing copy or fall outside a narrow scope.
5. **Trigger widened** — "what happened / went wrong / didn't work / error / rejected" now fire Reason (the user asked *"now what happened ?"* right after the failed submit and got a fast-path answer with **no capture** — the exact moment diagnosis was needed).
6. **Image tier default ON** for new workspaces (migration `20260715183302_reason_image_default_on`; Studio copy marks it *recommended*) — visual-only state (banners, met/unmet checklists) is where diagnosis quality lives; the founder still owns the toggle + disclosure.
7. **Look-before-concluding + no-speculation declines (2026-07-16, round two)** — the same duplicate-email state produced a decline ("looks fine — maybe server/network, check your internet") one minute before the correct banner-first diagnosis. Two binding prompt rules: the model may not conclude "everything looks fine" or decline until it has an on-page error to explain **or has examined the page image** (clean structure + a stuck user means the problem lives where structure can't see); and declines never invent causes — no server/network/connectivity speculation.
8. **Shared alert-surface detector; the fast path sees banners too (2026-07-16)** — `findAlertSurfaces` in `sense.ts` (standards signals + error-styled classes + **red-family text blocks** — the near-universal convention that catches utility-CSS banners like Tailwind's `text-red-600`, which carry no role and no "error" class) now feeds BOTH Reason's `[alert]` capture pass AND Sense's `findError`, so the masked rejection snippet rides the existing probe wire into **fast-path** answers — "whats next?" asked over a rejection banner no longer answers "go ahead and click it" (the miss: a fast-path follow-up 30s after the failed submit did exactly that, with zero capture).

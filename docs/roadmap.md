...
| **P4-M0b** | **First-Time User Onboarding** — the walkthrough's *pushed* trigger: a flagged workflow starts its walkthrough by itself when a new user lands on its first page; per-workflow flag on the approval row + workspace master switch (off by default) + the founder's per-browser show budget + a Studio overview of what fires and why not | ✅ **Done** — built + user-verified E2E 2026-09-11 (decisions: [`agent.md`](build/agent.md) §A8.1); needs Sense + the walkthrough switch; one additive migration; no end-user identity yet (per-browser "new"); activation analytics parked |
...
| Module | What it is | Status |
|:---|:---|:---|
| **V2 · P0…P6** | **Help Portal & Articles track** (ex-Phase 2, moved 2026-07-08) — publish foundation (per-audience approval + presentation overlay) · Text→Article · public portal · search UI · authoring depth · productization (incl. **PII Cut 2**: screenshot OCR/blur, gates publish) · coverage analytics + collaboration | 📝 **Draft** — [`portal.md`](build/portal.md), with the track's first slices **shipped ahead**: **2026-08-25 the SOP export** (per-workflow downloads in Studio's Video/SOP tab — a human guide zip + a text-only AI-agent manual, the [`interop.md`](build/interop.md) §5 instructional layer, compiled on request by the one SOP compiler every later transport renders) **and the public help portal v1** (P0 minimal + P2: `PortalPublication` on workflow identity with a founder-assigned category; workspace switch OFF by default behind a recorded screenshot acknowledgment — the pixel-PII posture until Cut 2; SSR portal at `/help/<slug>` — category index + instant client-side search, articles with demo video, highlight overlays, HowTo/Breadcrumb JSON-LD, per-portal sitemap, ISR + on-publish revalidation; detached/contentless workflows fail closed). **2026-08-26 the agent door on that portal:** every published article serves a `.md` sibling (the agent manual) plus a per-portal `llms.txt` index — the P6-M2b markdown rider, live on the portal's URL space; custom domains later put the same file at a domain root. **Same day, the Studio Help Portal manager** (left-nav page): switch + acknowledgment, guides grouped by category, bulk publish from the KB, category rename, and the fail-closed "Hidden — needs re-review" state made visible. Remaining here: presentation overlay + authoring depth, Text→Article, retrieval-backed search, productization (theming, custom domains, PII Cut 2), coverage analytics + collaboration |

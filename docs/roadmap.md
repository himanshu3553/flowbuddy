...
- **Boundary learning — "every founder correction into permanent product knowledge" — 📝 direction captured 2026-08-20, founder-committed for after the Reorganize surface ships.** Every boundary the founder draws or removes on the Reorganize surface is a lesson about where tasks start in *their* product; today it corrects one recording and is spent. The plan: derive a boundary **signature** from each correction (route pattern + control fingerprint — the Sense machinery), and on a **future** recording a matching event becomes a hard cut exactly like a founder-pressed marker — deterministic, structure-not-prompt, possibly launched as pre-marked *suggestions* before graduating to automatic. Secondary: a post-hoc "this looks like two tasks you keep separate — split here?" prompt via the existing overlap embeddings. Rejected: feeding learned boundaries to the segmenter as prompt hints (hands boundary judgment back to the model). The stored boundary lists are already the training data — nothing extra to capture in the meantime.

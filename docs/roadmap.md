...
├─ PHASE 3 · Self-validation & freshness (moat) 🟩🟨⬜                       1 done · 1 in progress · replay track to be planned
...
| **P4-M3** | **Contractual shell** — versioned terms acceptance (written in the same transaction that first sets the mode to `agent`, and required by the server on every later attempt), destructive-step confirms, takeover/abort, `ExecutionRun` audit + Analytics "Agent runs" | ✅ **Built + user-verified 2026-08-05** |

...
| **P1-M1** | Recorder / workflow capture (Chrome extension: events + DOM + screenshots + narration) | ✅ **Done** — **v0.6.0 "FlowBuddy Recorder" LIVE on the Chrome Web Store (2026-07-23)**: the production release — bakes `app.flowbuddyai.com` + the dev Studio + localhost, FlowBuddy "F" icons, carries **R13 ranked locators** (the Sense/Phase-3 enabler) + structured logging. Full log: [`extension-releases.md`](extension-releases.md). **⚠️ ACTION OPEN — v0.7.0 packaged 2026-07-28, NOT yet submitted, and the api that requires it is ALREADY in production.** The upload-identity release (direct artifact upload · the `X-FlowBuddy-Upload-Id` header · the discard route) exists as a build but has not been uploaded to the store. The intended store-first ordering was therefore inverted: `/v1/sessions` returns **400** without that header and **v0.6.0 — the build the store still serves — does not send it**, so a store-installed recorder cannot upload a recording at all. A local rebuild fixes it for developers only. **Submitting v0.7.0 is the release-blocking item.** | M2 |

...
| **P3-M1** | **Workflow identity & selection** (Cut 2 of the same spec) — a workflow gains a durable identity that outlives the recording slot it came from; a reprocess re-matches workflows to identities **by content**, so an approval survives only where the content still agrees and fails closed where it doesn't (new → unapproved, lost → `needs_review`). Siblings sharing a *task* then get **one** selected per task before ranking (screen match, else the generic one) | 🔄 **In Progress** — identity, content-matching, every gate AND every Studio mutation re-keyed; an approval no longer carries a position at all. **Built and user-verified E2E 2026-07-31; the reprocess hazard is CLOSED.** **Outstanding:** task grouping and one-per-task selection, so "both are real" still only records the decision. *(The widget wire still names workflows by position. That is now hygiene rather than correctness — the gate is identity-based, so a stale position is a wrong SIGNAL bounded by the sense plan's version hash and 60s TTL, never a leak — and nothing else waits on it.)* |

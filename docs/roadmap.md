...
- **Founder edits across re-recordings (captured 2026-08-21)** — content edits are scoped to the recording they were made on (a recording is a frozen product snapshot), so replacing a workflow via the duplicate flow loses them on the new telling. Two follow-ups: (1) **carry-forward on Replace** — match the old workflow's edited steps to the new one's by the same moment signature boundary learning uses (control label + screen), and OFFER to re-apply compatible text edits (and a stamped title/description) as a reviewable list, never silently — the product may have changed exactly that step; image picks can't transfer (frames belong to their recording). (2) **Reset to generated** per field — an edited title/description/step text is founder-owned until edited again; a reset should clear the stamp so the next re-process refreshes it from the model.

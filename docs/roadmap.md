...
| **P1-M1** | Recorder / workflow capture (Chrome extension: events + DOM + screenshots + narration) | ✅ **Done** — **v0.5.0 "FlowBuddy Recorder" submitted to the Chrome Web Store 2026-07-17 (in review)**: the rename release, carrying **R13 ranked locators** (the Sense/Phase-3 enabler) + structured logging; baked at the fresh `flowbuddy-dev-*` stack. Store v0.3.0 is still the live listing but its pre-rename baked URL is dead — installs reconnect once 0.5.0 ships. Full log: [`extension-releases.md`](extension-releases.md). | M2 |

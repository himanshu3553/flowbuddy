...
- **Recorder upload path (opened 2026-07-27 by the idempotent-upload change, `dev` only — not deployed)** — an **abandoned recording** now leaves a `recording` row plus orphaned objects in storage with **no cleanup path** (nothing sweeps or expires them); the presigned PUT URLs carry **no size ceiling** (`MAX_BUNDLE_BYTES` only ever covered the `/v1/sessions` bundle, so the direct-artifact path is unbounded); and **Cloudflare R2 + CORS on a browser-issued presigned PUT is unproven** — verified against MinIO only.
